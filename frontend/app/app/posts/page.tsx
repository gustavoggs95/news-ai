...
export default async function AppMyNews() {
  try {
    const reqHeaders = await headers();
    // Extract the host header (e.g., "example.com" or "localhost:3000")
    const host = reqHeaders.get("host");
    // Choose protocol based on your environment (adjust as needed)
    const protocol = process.env.NODE_ENV === "development" ? "http" : "https";
    // Construct the base URL
    const baseUrl = `${protocol}://${host}`;
    const response = await fluxApi.get(`${baseUrl}/api/news/myPosts`);
    const news = response.data.news as MyNews[];
    ...
    return <div>Error loading posts {getErrorMessage(error)}</div>;
  }
}
